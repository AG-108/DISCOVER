...
class Ceof(nn.Module):
    def __init__(self, coef_list, n_out=1):
        super().__init__()
        self.coeff_vector = nn.ParameterList(
            [torch.nn.Parameter(torch.FloatTensor(coef_list[i]).reshape(-1, 1)) for i in range(n_out)])
...
class PINN_model:
    ...
    def train_pinn_cv(self, program_pde, coef=1):
        ...
        for epoch in range(self.pinn_cv_epoch):
            ...
            if (epoch + 1) % 10 == 0:
                logging.info(
                    f"epoch: {epoch + 1}, mse: {loss_mse.item()}, res_loss1:{loss_res.item()}, total_loss:{loss.item()}")
        ...
        for epoch in range(self.pinn_epoch):
            ...
            for i in range(pde_num):
                ...
                if local_sample:
                    ...
                    loss_res2 = pinn_loss(self, program_pde[i], self.x_local, self.t_local, program_pde[i].w,
                                          extra_gradient=self.extra_gradient)
            ...
            if (epoch + 1) % 10 == 0:
                logging.info(
                    f"epoch: {epoch + 1}, mse: {loss_mse.item()}, pde_loss:{pde_loss.item()}, total_loss:{loss.item()}")
    ...
    def pinn_loss_explicitly(self, x_f, t_f):
        ...
        rhsu = -0.018939 * u[:, 0:1] - 0.983833 * u[:, 0:1] * u[:,
                                                              1:2] ** 2 + 0.019415 * uxx + 0.019880 * uyy + 0.016247 * uzz + 0.014109
        rhsv = 1.015508 * u[:, 0:1] * u[:, 1:2] ** 2 - 0.071700 * u[:,
                                                                  1:2] + 0.009610 * vyy + 0.009744 * vxx + 0.007907 * vzz
    ...
    def set_optimizer(self, coef_lr=0.001):
        ...
        self.optimizer = torch.optim.Adam([{'params': self.nn.parameters(), 'lr': 0.001},
                                           {'params': self.coef.coeff_vector.parameters(), 'lr': coef_lr}])
    ...
    def reconstructed_field_evaluation(self, path=None):
        ...
        if u_pred.shape[1] > 1:
            for i in range(1, u_pred.shape[1]):
                ...
                np.savez(path + f'/predicted{i}.npz', u_pred=tensor2np(u_pred1), x_star=tensor2np(self.x_star),
                         u_star=self.u_star)
