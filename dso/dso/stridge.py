...
class Node(object):
    ...
    def __repr__(self):
        ...
        if len(self.children) == 0:
            return self.val  # Avoids unnecessary parentheses, e.g. x1()
...
class Regulations(object):
    """
    set regulations to double check invalid expressions
    ...
    def apply_regulations(self, x, traversal, terms_token, depth):
        ...
        # check and omit terms that has inconsistent dimension
        ##############################################
...
class STRidge(object):
    ...
    def split_forest(self):
        ...
        def max_depth(root):
            """calculate the max depth of subtrees
    ...
    def calculate_RHS_terms(self, u, x, execute_func=unsafe_execute_torch, extra_gradient=False):
        ...
        # return the result
        # Args:
        #     u (list): list of state variables = [u1, u2, ...]
        #     x (list):  list of spatial variables = [x1, x2, ...]
        #     execute_function (function):  function to execute the program
        ##############################################
        results = []
    ...
    def evaluate_terms(self, u, x, test=False, execute_function=unsafe_execute):
        ##############################################
        # evaluate the function terms and emit invalid terms
        # return the results of each term
        # Args:
        #     u (list): list of state variables = [u1, u2, ...]
        #     x (list):  list of spatial variables = [x1, x2, ...]
        #     test (bool):  whether to cut off results on the boundary
        #     execute_function (function):  function to execute the program
        ##############################################
        results = []
