...
class PDEPINNTask(PDETask):
    ...
    def __init__(self, function_set, dataset, metric="residual",
        ...
        if "MD_NU" in dataset:
            ...
            n_state_var = 1
        self.u, self.x, self.t, ut, sym_true, n_input_var, test_list, n_state_var = load_class(dataset,
                                                                                               data_noise_level,
                                                                                               data_amount,
                                                                                               True,
                                                                                               cut_ratio=cut_ratio)
        self.u_true, self.x, self.t, ut_true, sym_true, n_input_var, test_list, _ = load_class(dataset, 0,
                                                                                               data_amount,
                                                                                               True,
                                                                                               cut_ratio=cut_ratio)
    ...
    def generate_meta_data(self, model, generation_type='AD', plot=False):
        ...
        FD refers to the finite difference. Only generate data on regular grids
        AD refers to the automatic differentiation. Use collocation data as training data
        Args:
            model (_type_): NN
        ...
        if generation_type == "AD" or generation_type == "FD":
            ...
            pass
            # TODO: finish the part for higher dimension
    ...
    def FD_generate_2d(self, u):
        ...
        uw, sigmaw, vw = nl.svd(W, full_matrices=False);
        vw = vw.T
        uu, sigmau, vu = nl.svd(U, full_matrices=False);
        vu = vu.T
        uv, sigmav, vv = nl.svd(V, full_matrices=False);
        vv = vv.T

        uwn, sigmawn, vwn = nl.svd(Wn, full_matrices=False);
        vwn = vwn.T
        uun, sigmaun, vun = nl.svd(Un, full_matrices=False);
        vun = vun.T
        uvn, sigmavn, vvn = nl.svd(Vn, full_matrices=False);
        vvn = vvn.T
        ...
        Wn = uwn[:, 0:dim_w].dot(np.diag(sigmawn[0:dim_w]).dot(vwn[:, 0:dim_w].T)).reshape(n, m, steps).transpose(2, 0,
                                                                                                                  1)
        Un = uun[:, 0:dim_u].dot(np.diag(sigmaun[0:dim_u]).dot(vun[:, 0:dim_u].T)).reshape(n, m, steps).transpose(2, 0,
                                                                                                                  1)
        Vn = uvn[:, 0:dim_v].dot(np.diag(sigmavn[0:dim_v]).dot(vvn[:, 0:dim_v].T)).reshape(n, m, steps).transpose(2, 0,
                                                                                                                  1)
        self.u_new = [Wn, Un, Vn]
        ...
        for idx in range(t_len - 1):
            self.ut_new[idx, :, :] = (self.u_new[0][idx + 1, :, :] - self.u_new[0][idx, :, :]) / (
                dt)  # FiniteDiff(self.u[0][idx, :], dt)
