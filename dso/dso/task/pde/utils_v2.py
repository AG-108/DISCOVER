...
def forward_diff(u, dx, axis=0):
    ...
    n = u.shape[axis]
    ux = np.zeros_like(u)
    ux[(slice(None),) * (axis % ux.ndim) + (slice(1, n),)] \
...
def backward_diff(u, dx, axis=0):
    ...
    n = u.shape[axis]
    ux = np.zeros_like(u)
    ux[(slice(None),) * (axis % ux.ndim) + (slice(0, n-1),)] \
        = (array_slice(u, axis, 1, n) - array_slice(u, axis, 0, n-1)) / dx
    ux[(slice(None),) * (axis % ux.ndim) + (n-1,)] \
