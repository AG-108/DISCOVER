...
def load_2d2U_data(dataset,
    ...
    if dataset == 'Allen_Cahn_2D':
        ...
        # with corresponding x, y, and t values.
        ...
        # Extract the lower and upper bounds for each entry
        lb = X_star.min(0)
        ub = X_star.max(0)
        ...
        # add noise
        u_star_train = u_star_train + noise_level * np.std(u_star_train) * np.random.randn(u_star_train.shape[0],
                                                                                           u_star_train.shape[1])
        v_star_train = v_star_train + noise_level * np.std(v_star_train) * np.random.randn(v_star_train.shape[0],
                                                                                           v_star_train.shape[1])
        u_star_val = u_star_val + noise_level * np.std(u_star_val) * np.random.randn(u_star_val.shape[0],
                                                                                     u_star_val.shape[1])
        v_star_val = v_star_val + noise_level * np.std(v_star_val) * np.random.randn(v_star_val.shape[0],
                                                                                     v_star_val.shape[1])
