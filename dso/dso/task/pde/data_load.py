...
def load_data_2D(dataset, noise_level=0, data_amount=1, training=False, cut_ratio=0.03):
    ...
    if noise_level > 0:
        # import pdb;pdb.set_trace()
        t_len, n, m = ut.shape
        ut = ut[math.floor(t_len * 0.1):math.ceil(t_len * 0.9), math.floor(n * 0.1):math.ceil(n * 0.9),
             math.floor(m * 0.1):math.ceil(m * 0.9)]
...
def load_data_MD_NU(dataset, noise_level=0, data_amount=1, training=False, cut_ratio=0.03):
    if "2D" in dataset:
        ...
        if noise_level > 0 and training:
            # ut = ut[math.floor(n*0.1):math.ceil(n*0.9), math.floor(m*0.1):math.ceil(m*0.9)]
            ut = ut[math.floor(t_len * 0.03):math.ceil(t_len * 0.97), math.floor(n * 0.03):math.ceil(n * 0.97),
                 math.floor(m * 0.03):math.ceil(m * 0.97)]
        ...
        if dataset == 'ns_MD_NU':
            ...
            if noise_level > 0 and training:
                # ut = ut[math.floor(n*0.1):math.ceil(n*0.9), math.floor(m*0.1):math.ceil(m*0.9)]
                ut = ut[math.floor(steps * 0.03):math.ceil(steps * 0.97), math.floor(n * 0.03):math.ceil(n * 0.97),
                     math.floor(m * 0.03):math.ceil(m * 0.97)]
            X = [x_data, y_data]
            n_input_var = 2
            n_state_var = 3
            ...
            if noise_level > 0 and training:
                # ut = ut[math.floor(n*0.1):math.ceil(n*0.9), math.floor(m*0.1):math.ceil(m*0.9)]
                ut = ut[math.floor(steps * cut_ratio):math.ceil(steps * (1 - cut_ratio)),
                     math.floor(n * cut_ratio):math.ceil(n * (1 - cut_ratio)),
                     math.floor(m * cut_ratio):math.ceil(m * (1 - cut_ratio))]
            X = [x_data, y_data]
            n_input_var = 2
